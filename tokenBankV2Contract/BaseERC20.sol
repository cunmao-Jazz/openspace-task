...
contract BaseERC20 {
    ...
    function transferWithCallback(address recipient, uint256 amount) external AddressVerification(recipient) returns(bool){
        transfer(msg.sender, amount);

       if (recipient.isContract()){
            (bool success,) = recipient.call(
                abi.encodeWithSignature("tokensReceived(address,uint256)", msg.sender, amount)
            );
